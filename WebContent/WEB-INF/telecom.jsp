...
<body>  <!-- onload=" getTableContent('tb')" -->
	...
	<table    cellpadding="0" cellspacing="0" id="tb" class="table table-striped" >
		...
		<c:forEach items="${telecom}" var="telecom"  varStatus="loop" >
	
		<tr>
		    <td>${telecom.telecom_college}</td>
			<td>${telecom.telecom_major}</td>
			 <td class="count1" >${telecom.telecom_graduate_count}</td> 
			<td class="count2">${telecom_gra[loop.count-1].telecom_employment_count}</td>
            <td><fmt:formatNumber type="number" value="${telecom_gra[loop.count-1].telecom_employment_count/ telecom.telecom_graduate_count*100} " maxFractionDigits="2"/>%</td>
      	</tr>
