...
<body>
	...
	<table    cellpadding="0" cellspacing="0" id="tb" class="table table-striped" >
		<caption>船舶与建筑工程学院专业就业率</caption>
		<tr>
		    <td>学院</td>
			<td>专业</td>
			<td>毕业人数</td>
		...
		<c:forEach items="${ship}" var="ship"  varStatus="loop" >
	
		<tr>
		    <td>${ship.ship_college}</td>
			<td>${ship.ship_major}</td>
			 <td class="count1" >${ship.ship_graduate_count}</td> 
			<td class="count2">${ship_gra[loop.count-1].ship_employment_count}</td>
            <td><fmt:formatNumber type="number" value="${ship_gra[loop.count-1].ship_employment_count/ ship.ship_graduate_count*100} " maxFractionDigits="2"/>%</td>
      	</tr>
